...
% morewrites/scrwfile allow to open more than 16 files concurrently, see
% https://latex.org/forum/viewtopic.php?t=12655
% We choose to use scrwfile because the morewrites package seems to have some bugs.
\usepackage{scrwfile}
...
\newcommand{\codesystem}{listings} % Valid options are biblatex or bibtex
% \newcommand{\codesystem}{minted} % Valid options are biblatex or bibtex

\ifthenelse{\equal{\codesystem}{listings}}
{
  \usepackage{listings}
...
% Symbols and acronym's backend selection. Note that the makeglossaries
% requires an external program and makenoidxglossaries may have problems when
% using UTF-8 encoded acronyms.
